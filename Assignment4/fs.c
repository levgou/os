...
stati(struct inode *ip, struct stat *st)
{
  ...
  if (ip->major == PROCFS )
    st->dev_t = devsw[PROCFS].isdir(ip) ? DEV_DIR : DEV_FILE;
  else
    st->dev_t = -1;
...
void print_inodes_info() {
  struct inode *ip;
  acquire(&icache.lock);
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
    if(ip->ref > 0) {
      cprintf("<%d> <%d> <%d> <%d> <(%d, %d)> <%d> <%d>\n",
              ip->dev, ip->inum, ip->valid, ip->type, ip->major, ip->minor, ip->nlink, ip->size);
    }
  }
  release(&icache.lock);
}